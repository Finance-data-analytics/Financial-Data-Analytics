...
class ScreencastFrameMetadata:
    ...
    #: Position of horizontal scroll in static pixels.
    scroll_offset_x: float

    #: Position of vertical scroll in static pixels.
...
class LayoutViewport:
    ...
    '''
    #: Horizontal offset relative to the document (static pixels).
    page_x: int

    #: Vertical offset relative to the document (static pixels).
    page_y: int

    #: Width (static pixels), excludes scrollbar if present.
    client_width: int

    #: Height (static pixels), excludes scrollbar if present.
    client_height: int
...
class VisualViewport:
    ...
    '''
    #: Horizontal offset relative to the layout viewport (static pixels).
    offset_x: float

    #: Vertical offset relative to the layout viewport (static pixels).
    offset_y: float
    ...
    #: Page zoom factor (static to device independent pixels ratio).
...
def get_layout_metrics() -> typing.Generator[T_JSON_DICT,T_JSON_DICT,typing.Tuple[LayoutViewport, VisualViewport, dom.Rect, LayoutViewport, VisualViewport, dom.Rect]]:
    ...
    :returns: A tuple with the following items:
        ...
        3. **cssLayoutViewport** - Metrics relating to the layout viewport in static pixels.
        4. **cssVisualViewport** - Metrics relating to the visual viewport in static pixels.
        5. **cssContentSize** - Size of scrollable area in static pixels.
    '''
    cmd_dict: T_JSON_DICT = {
...
def set_device_metrics_override(
    ...
    window.innerWidth, window.innerHeight, and "device-width"/"device-height"-related static media
    query results).
