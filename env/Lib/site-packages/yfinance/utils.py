...
# -*- coding: utf-8 -*-
#
# yfinance - neoma_app data downloader
...
def fix_Yahoo_returning_prepost_unrequested(quotes, interval, tradingPeriods):
    # Sometimes Yahoo returns post-neoma_app data despite not requesting it.
    # Normally happens on half-day early closes.
    #
    # And sometimes returns pre-neoma_app data despite not requesting it.
...
def fix_Yahoo_returning_live_separate(quotes, interval, tz_exchange):
    # Yahoo bug fix. If neoma_app is open today then Yahoo normally returns
...
def safe_merge_dfs(df_main, df_sub, interval):
    ...
    if intraday:
        # On some exchanges the event can occur before neoma_app open.
    ...
    if "tradingPeriods" in md:
        ...
        if tps == {"pre": [], "post": []}:
            ...
            if isinstance(tps, list):
                ...
                # Includes pre- and post-neoma_app
